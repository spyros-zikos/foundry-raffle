...
contract FundSubscription is Script {
    uint96 public constant FUND_AMOUNT_SEPOLIA = 0.3 ether;
    uint96 public constant FUND_AMOUNT_ANVIL = 3 ether;
    ...
    function fundSubscription(address vrfCoordinator, uint64 subId, address link, uint256 deployerKey) public {
        ...
        if (block.chainid == 31337) {
            vm.startBroadcast(deployerKey);
            VRFCoordinatorV2Mock(vrfCoordinator).fundSubscription(subId, FUND_AMOUNT_ANVIL);
            vm.stopBroadcast();
        } else {
            vm.startBroadcast(deployerKey);
            LinkToken(link).transferAndCall(vrfCoordinator, FUND_AMOUNT_SEPOLIA, abi.encode(subId));
            vm.stopBroadcast();
        }
    }
